<div class="newsletter-module<?= $uniqid ?> newsletter-module">
    <form id="form-newsletter<?= $uniqid ?>">
    	<h3><?= $title ?></h3>
        <?php if($description) { ?>
        <p class="pd-b15 text-center"><?= $description ?></p>
        <?php } ?>
		...
		<div class="flex input-wrap">
			...
			<div class="text-center text-sm-right btc"><button type="button" id="submit-newsletter<?= $uniqid ?>" data-loading-text="<?= $text_loading; ?>" class="btn btn-primary"><?= $submit_button_field_text; ?></button></div>
		</div>
		<?php } ?>
	</form>
</div>
