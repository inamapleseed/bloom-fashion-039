...
<?php if(!$not_avail) { ?>
    ...
    <?php if ($option['type'] == 'color') { ?>
      <div class="form-group<?= ($option['required'] ? ' required' : ''); ?> opt">
        <label><?= $option['name']; ?></label>

        <div id="input-option<?= $option['product_option_id']; ?>" class="option-color">
          <?php foreach ($option['product_option_value'] as $option_value) { ?>
            <div class="radio">
              <label class="radio-container">
                <?php if ($color_id == $option_value['product_option_value_id']): ?>
                  <input type="radio" name="option[<?= $option['product_option_id']; ?>]" id="selected_color" value="<?= $option_value['product_option_value_id']; ?>" data-id="<?= $option_value['color']; ?>" selected checked />
                <?php else: ?>
                    <input type="radio" name="option[<?= $option['product_option_id']; ?>]" value="<?= $option_value['product_option_value_id']; ?>" data-id="<?= $option_value['color']; ?>"/>
                <?php endif ?>
                <div class="checkmark" style="background-color:<?= $option_value['color']; ?>"></div>
             </label>
            </div>
          <?php } ?>
        </div>
    ...
    <?php if ($option['type'] == 'radio') { ?>
      <div class="form-group<?= ($option['required'] ? ' required' : ''); ?> opt">
        <label><?= $option['name']; ?></label>
        <div id="input-option<?= $option['product_option_id']; ?>" class="rad-con">
          <?php foreach ($option['product_option_value'] as $option_value) { ?>
            <div class="radio">
              <label>
          ...
          <?php if($attribute_groups): ?>
              <?php foreach ($attribute_groups as $attribute_group): ?>
                  <?php if ($attribute_group['name'] == 'Size Guide'): ?>
                    <?php if ($option['name'] == 'Size' || $option['name'] == 'Sizes' || $option['name'] == 'size' || $option['name'] == 'sizes'): ?>
                      <button type="button" class="btn btn-fancy" data-toggle="modal" data-target="#sizeguide">
                        <?php echo $attribute_group['name']; ?>
                      </button>
        
                      <div class="modal fade" id="sizeguide" tabindex="-1" role="dialog" aria-labelledby="sguide" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h2 class="modal-title" id="sguide"><?php echo $attribute_group['name']; ?></h2>
                              <p><?php echo $attribute_group['mydescription']; ?></p>
