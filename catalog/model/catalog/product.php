...
class ModelCatalogProduct extends Model {
	...
	public function getFilterByProducts($filters = array()){
		...
		if ($implode) {
			...
			foreach ($filter_group_query->rows as $filter_group) {
				...
				$filter_query = $this->db->query("SELECT DISTINCT f.filter_id, fd.name, fd.color FROM " . DB_PREFIX . "filter f LEFT JOIN " . DB_PREFIX . "filter_description fd ON (f.filter_id = fd.filter_id) WHERE f.filter_id IN (" . implode(',', $implode) . ") AND f.filter_group_id = '" . (int)$filter_group['filter_group_id'] . "' AND fd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY f.sort_order, LCASE(fd.name)");
	...
	public function getProductAttributes($product_id) {
		...
		$product_attribute_group_query = $this->db->query("SELECT ag.attribute_group_id, agd.name, agd.mydescription FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE pa.product_id = '" . (int)$product_id . "' AND agd.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY ag.attribute_group_id ORDER BY ag.sort_order, agd.name, agd.mydescription");
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			foreach ($product_attribute_query->rows as $product_attribute) {
				...
				// 'mydescription'         => $product_attribute['mydescription'],
			...
			'mydescription'               => $product_attribute_group['mydescription'],
	...
	public function getProductColorImage($product_id, $product_option_id, $product_option_value_id) {
		$sql = "SELECT * FROM " . DB_PREFIX . "product_option_image WHERE product_id = '" . (int)$product_id . "' AND product_option_id = '" . (int)$product_option_id . "' AND product_option_value_id = '" . (int)$product_option_value_id . "' ORDER BY sort_order ASC LIMIT 2";
		$query = $this->db->query($sql);
		return $query->rows;
	}
    // >>OPTIONS IMAGE
