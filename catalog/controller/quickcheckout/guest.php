...
class ControllerQuickCheckoutGuest extends Controller {
  	public function index() {
		...
		if (isset($this->session->data['payment_address']['unit_no'])) {
			$data['unit_no'] = $this->session->data['payment_address']['unit_no'];
		} else {
			$data['unit_no'] = '';
		}
		...
		if (isset($this->session->data['guest']['custom_field'])) {
			$data['guest_custom_field'] = $this->session->data['guest']['custom_field'] + (isset($this->session->data['payment_address']['custom_field']) ? $this->session->data['payment_address']['custom_field'] : array());
		} else {
	...
	public function validate() {
		...
		if (!$json) {
			...
			$unit_no = $this->config->get('quickcheckout_field_unit_no');

			if (!empty($unit_no['required'])) {
				if ($this->request->post['unit_no'] == '') {
					$json['error']['unit_no'] = $this->language->get('error_unit_no');
				}
			}
		...
		if (!$json) {
			...
			$this->session->data['payment_address']['unit_no'] = $this->request->post['unit_no'];
			...
			if ($this->session->data['guest']['shipping_address']) {
				...
				$this->session->data['shipping_address']['unit_no'] = $this->request->post['unit_no'];
