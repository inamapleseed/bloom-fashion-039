    ...
    class ControllerComponentProductInfo extends Controller{
        public function index($product_id = 0){
            ...
            $image = $this->model_tool_image->resize('no_image.png', $width, $height);
             ...
             $image_hover = $this->model_tool_image->resize('no_image.png', $width, $height);
            $image2 = false;
            ...
            if (is_file(DIR_IMAGE . $product_info['image_hover']) && $product_info['image_hover']) 
                $image_hover = $this->model_tool_image->resize($product_info['image_hover'], $width, $height);
            ...
            $color_option_infos = $this->model_catalog_product->getProductColorOptions($product_id);
            if($color_option_infos){
                if(isset($color_option_infos[0]['product_option_value'])){
                    
                }

                

                foreach($color_option_infos[0]['product_option_value'] as $color_option_info){
                    $image_opt = $this->model_catalog_product->getProductColorImage($product_id,$color_option_infos[0]['product_option_id'],$color_option_info['product_option_value_id']);
                    $image_optimg="";
                    $image_optimg2="";
                    if ($image_opt) {
                        if (isset($image_opt[0]['image']) && is_file(DIR_IMAGE . $image_opt[0]['image'])){
                            // $image_optimg = $this->model_tool_image->resize($image_opt[0]['image'], $width, $height);
                            $image_optimg = 'image/'.$image_opt[0]['image'];
                        }
                    }

                    if ($image_opt) {
                        if (isset($image_opt[1]['image']) && is_file(DIR_IMAGE . $image_opt[1]['image'])){
                            $image_optimg2 = 'image/'.$image_opt[1]['image'];
                        }
                    }
                    ...
                    $color_option_array[] = array(
                        'name'                      => $color_option_info['name'],
                        'color'                      => $color_option_info['color'],
                        'product_option_id'         => $color_option_infos[0]['product_option_id'],
                        'product_option_value_id'   => $color_option_info['product_option_value_id'],
                        'option_value_id'           => $color_option_info['option_value_id'],
                        'image'                     => $image_optimg,
                        'image2'                     => $image_optimg2
                    );
                }
            }
            $category = array();
            ...
            $info = array(
                ...
                'special'           => $special,
                'tax'               => $tax,
                'color_options'     =>  $color_option_array,
                'rating'            => $rating,
                'href'              => $product_url,
