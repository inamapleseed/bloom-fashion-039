...
class ModelCatalogAttributeGroup extends Model {
	public function addAttributeGroup($data) {
		...
		foreach ($data['attribute_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_group_description SET attribute_group_id = '" . (int)$attribute_group_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', mydescription = '" . $this->db->escape($value['mydescription']) . "'");
		}
	...
	public function getAttributeGroupDescriptions($attribute_group_id) {
		...
		foreach ($query->rows as $result) {
			$attribute_group_data[$result['language_id']] = array('name' => $result['name'], 'mydescription' => $result['mydescription']);
		}

		return $attribute_group_data;
	}
	// public function getAttributeGroupDescriptions2($attribute_group_id) {
	// 	$attribute_group_data2 = array();

	// 	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_group_description WHERE attribute_group_id = '" . (int)$attribute_group_id . "'");
	// 	foreach ($query->rows as $result) {
	// 	}

	// 	return $attribute_group_data2;
	// }
